...
const RecentHospitalizations: React.FC<RecentHospitalizationsProps> = ({ hospitalizations }) => {
  ...
  const getHospitalizationPaymentImage = (status: number | string) => {
    switch (status) {
      case 0:
        return 'bg-blue-400';
      case 1:
  ...
  const getHospitalizationPaymentStatus = (status: number | string) => {
    switch (status) {
      case 0:
        return 'Pending';
      case 1:
  ...
  return (
    <div className='space-y-8'>
      {hospitalizations.map((hospitalization) => (
        <div className='flex items-center' key={hospitalization.hospitalizationId}>
          ...
          <div className='ml-4 space-y-1 w-[25vw]'>
            <p className='text-sm font-medium leading-none flex justify-between'>
              ...
              <span>
                from {hospitalization.admissionDate} to {hospitalization.dischargeDate}
              </span>
            </p>
          ...
          <Badge
            className={
              getHospitalizationPaymentImage(hospitalization.paymentStatus) + ' text-white p-2 ml-auto w-[7vw]'
            }
            variant='secondary'
          >
            Payment: {getHospitalizationPaymentStatus(hospitalization.paymentStatus) ?? 'Cash'}
          </Badge>
          <div className='ml-auto font-medium'>+${hospitalization.totalCost}</div>
        </div>
      ))}
    </div>
  );
