...
const BookingForm: React.FC<BookingFormProps> = ({ userName, date, slot, onCancel }) => {
  ...
  const [selectedVetUserName, setSelectedVetUserName] = useState<string | null>(null);
    ...
    <div className='flex items-center justify-center h-full w-full'>
      <div className='w-full max-w-screen-md'>
        <div className='flex flex-col  justify-center items-center p-8 bg-white rounded-md shadow-md'>
          <h1 className='text-2xl font-bold mb-6 text-custom-pink'>Booking Details</h1>
          <form onSubmit={handleSubmit(onSubmit, onError)} className='w-full flex flex-col items-center'>
            <div className='flex flex-col mb-6 min-w-full items-center'>
              <h2 className='text-lg font-bold text-custom-blue mb-3'>Choose a pet to see the vet</h2>
              {pets.length > 0 ? (
            ...
            <div className='flex flex-col mb-6 min-w-full items-center'>
              <h2 className='text-lg font-bold text-custom-blue mb-3'>Available Services</h2>
              {services.length > 0 ? (
                <BookingService services={services} onSelectService={handleSelectService} />
              ) : (
                <p className='text-custom-blue'>No available services.</p>
              )}
            </div>
            <div className='flex flex-col mb-6 min-w-full items-center'>
              <h2 className='text-lg font-bold text-custom-blue mb-3'>Available Vets</h2>
              {vets.length > 0 ? (
                <BookingVet vets={vets} onSelectVet={handleSelectVet} />
              ) : (
                <p className='text-custom-blue'>No vets available for this date and timeslot.</p>
              )}
            </div>
