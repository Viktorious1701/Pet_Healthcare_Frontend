...
const RecentAppointments: React.FC<RecentAppointmentsProps> = ({ appointments }) => {
  appointments = appointments.filter((appointment) => new Date(appointment.date).getMonth() === now.getMonth());
      ...
      {appointments.map((appointment) => (
        <div className='flex items-center' key={appointment.appointmentId}>
          <Avatar className='h-9 w-9'>
            ...
            <AvatarFallback className='font-bold text-xs'>{appointment.vet.slice(0, 2).toUpperCase()}</AvatarFallback>
          </Avatar>
          ...
          <Badge
            className={getAppointmentImage(appointment.status) + ' text-white mx-2 p-2 w-[7vw]'}
            variant='secondary'
          >
            Status: {appointment.status}
          </Badge>
          <Badge
            className={getAppointmentPaymentImage(appointment.paymentStatus) + ' text-white p-2 w-[7vw] mr-2'}
            variant='secondary'
          >
            Payment: {getAppointmentPaymentStatus(appointment.paymentStatus) ?? 'Cash'}
          </Badge>
