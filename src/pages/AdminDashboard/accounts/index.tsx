          ...
          <TabsContent value='overview' className='space-y-4'>
            <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
              ...
              <Card>
                ...
                <CardContent>
                  <div className='text-2xl font-bold'>{Math.round((activeUsers / users.length) * 100)}%</div>
                </CardContent>
              </Card>
              <Card>
                <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
                  <CardTitle className='text-sm font-medium'>Users Roles Breakdown</CardTitle>
                  <Icon123 />
                </CardHeader>
                <CardContent>
                  <div className='text-2xl font-bold flex justify-between'>
                    <span>{users.filter((user) => user.role === 'Customer').length} Customers</span>
                    <span>
                      {((users.filter((user) => user.role === 'Customer').length / users.length) * 100)
                        .toString()
                        .slice(0, 5)}{' '}
                      % of system accounts
                    </span>
