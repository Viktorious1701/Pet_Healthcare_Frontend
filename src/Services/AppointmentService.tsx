...
import { AppointmentAvailableVets, AppointmentBook, AppointmentGet, AppointmentRating } from '@/Models/Appointment';
import { AppointmentDetailGet } from '@/Models/AppointmentDetail';
import { RecordGet } from '@/Models/Record';
...
export const appointmentAvailableVetsAPI = async (date: string, slotId: number) => {
  try {
...
export const getAppointmentByIdAPI = async (appointmentId: number) => {
  try {
    const data = await axiosInstance.get<AppointmentGet>(api + `/${appointmentId}`);
    return data;
  } catch (error) {
    handleError(error);
  }
...
export const appointmentCustomerAPI = async (username: string) => {
  try {
    const data = await axiosInstance.get<AppointmentGet[]>(api + `/customer/${username}`);
    return data;
  } catch (error) {
    handleError(error);
  }
...
export const appointmentVetAPI = async (vetId: string) => {
  try {
    const response = await axiosInstance.get<AppointmentGet[]>(api + `/vet/${vetId}`);
...
export const appointmentDetailsAPI = async (appointmentId: string) => {
  try {
    const response = await axiosInstance.get<AppointmentDetailGet>(apiAppointmentDetails + `/${appointmentId}`);
...
export const appointmentPostAPI = async (appointmentDetails: {
...
export const recordGetAPI = async () => {
  try {
    const data = await axiosInstance.get<RecordGet[]>(apiGetVet + `/Record/records`);
    return data;
  } catch (error) {
    handleError(error);
  }
