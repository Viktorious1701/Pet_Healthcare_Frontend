      ...
      renderValue={(items) => {
        return items.map((item) => (
          <div key={item.key} className='flex items-center gap-2'>
            <Avatar alt={item.data?.userName} className='flex-shrink-0' size='sm' />
            <div className='flex flex-col'>
              {item.data?.id === 'auto-assign' ? (
                <span className='text-small'>{'Let us choose'}</span>
              ) : (
                <span className='text-small'>{'Dr. ' + item.data?.firstName + ' ' + item.data?.lastName}</span>
              )}
          ...
          <div className='flex gap-2 items-center'>
            ...
            <div className='flex flex-col'>
              {user.id === 'auto-assign' ? (
                ...
                <span className='text-small'>{'Dr. ' + user.firstName + ' ' + user.lastName}</span>
              )}
