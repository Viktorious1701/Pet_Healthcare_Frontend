...
const BraintreeDropIn: React.FC<BraintreeDropInProps> = (props) => {
    ...
    if (braintreeInstance) {
      braintreeInstance.requestPaymentMethod((error, payload) => {
        if (error) {
          ...
          axiosInstance
            .post(`https://pethealthcaresystem.azurewebsites.net/api/Payment/Checkout`, {
  ...
  return (
    <div style={{ display: show ? 'block' : 'none' }}>
      ...
      <Button className='braintreePayButton bg-custom-pink' disabled={!braintreeInstance} onClick={handlePayment}>
        Pay
      </Button>
    </div>
  )
